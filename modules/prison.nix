...
{ config, lib, pkgs, knotLib, prison, ... }:
...
let
  ...
  # ...and the same configuration with the host paths, which is what the
  # start-time check has to resolve: it runs on the host, where the secrets
  # are, before the container exists.
  hostBuilt = knotLib.mkSettings { keyFiles = cfg.tsigKeyFiles; user = null; logTarget = "stdout"; };
  tsigPreflight = knotLib.mkPreflight hostBuilt.configFile;

  # A remote is written `address@port`; egress is opened to exactly those.
  splitRemote = a:
    let m = builtins.match "([^@]+)@([0-9]+)" a; in
    if m == null then { address = a; port = 53; }
    else { address = builtins.elemAt m 0; port = lib.toInt (builtins.elemAt m 1); };

  peerAddresses = lib.unique (lib.concatMap (r: r.address)
  ...
  egressTargets = lib.concatMap
    (a: let t = splitRemote a; in [
      { inherit (t) address port; protocol = "udp"; }
      { inherit (t) address port; protocol = "tcp"; }
    ])
    peerAddresses;
  ...
  knotd = prison.mkPrisonService {
    name = "knotd";
    exec = [ "${cfg.package}/bin/knotd" "--config" "/config/knot.conf" ];
    uid = 1000;
    ...
    persist =
      ...
      [ { host = cfg.stateDir; path = "/var/lib/knot"; } ]
      # TSIG secrets: read-only, one file each, never in the store.
      ++ map
        (f: { host = f; path = secretPath f; readOnly = true; file = true; })
        cfg.tsigKeyFiles;

    config = { "knot.conf" = built.configFile; };
...
in
{
  config = mkIf (cfg.enable && cfg.backend == "prison") {
    services.prisons.${cfg.containerName} = prison.mkPrison {
      ...
      listen = { tcp = tcpPorts; udp = udpPorts; };
      egress =
        if egressTargets == [ ]
        then { mode = "none"; targets = [ ]; lan = [ ]; }
        else { mode = "targets"; targets = egressTargets; lan = [ ]; };
    };
    ...
    systemd.services."${cfg.containerName}".serviceConfig.ExecStartPre =
      lib.mkBefore [ "${tsigPreflight}" ];
  };
}
